...
import {
	...
	useNavigateToMapView,
} from "@/hooks/helpers/use-navigate";
...
export const TopLeftNavigation = memo(
	({ structures }: { structures: PlayerStructure[] }) => {
		const {
			setup,
			account: { account },
		} = useDojo();
		...
		const structure = useMemo(() => {
			return {
				...entityInfo,
				isFavorite: favorites.includes(entityInfo.entityId),
			};
		}, [structureEntityId, favorites, entityInfo]);

		const structurePosition = useMemo(() => {
			return new Position(
				structure?.position || { x: 0, y: 0 },
			).getNormalized();
		}, [structure]);
		...
		const toggleFavorite = useCallback((entityId: number) => {
			setFavorites((prev) => {
				const newFavorites = prev.includes(entityId)
		...
		const goToHexView = useCallback(
			(entityId: ID) => {
				...
				navigateToHexView(
					new Position({
						x: structurePosition.coord_x,
						y: structurePosition.coord_y,
					}),
				);
			},
			[navigateToHexView],
		...
		const goToMapView = useCallback(
			(entityId?: ID) => {
				...
				if (!position) return;
				navigateToMapView(
					new Position({ x: position.coord_x, y: position.coord_y }),
				);
			},
		...
		const { progress } = useMemo(() => {
			const timeLeft =
				currentBlockTimestamp % configManager.getTick(TickIds.Armies);
			const progressValue =
				(timeLeft / configManager.getTick(TickIds.Armies)) * 100;
										...
										{structuresWithFavorites.map((structure, index) => (
											<div key={index} className="flex flex-row items-center">
												<button
													...
													type="button"
													onClick={() =>
														toggleFavorite(structure.structure.entity_id)
													}
												>
													{
														<Star
												...
												<SelectItem
													...
													key={index}
													value={
														structure.structure.entity_id?.toString() || ""
													}
												>
	...
	const tooltipContent = useMemo(
		() => (
			<div className="whitespace-nowrap pointer-events-none flex flex-col mt-3 mb-3 text-sm capitalize">
				<div>
					A day in Eternum is{" "}
					<span className="font-bold">{formatTime(cycleTime)}</span>
				</div>
				<div>
					Time left until next cycle:{" "}
					<span className="font-bold">
						{formatTime(cycleTime - (currentBlockTimestamp % cycleTime))}
					</span>
				</div>
			</div>
		),
