...
import type { ContractComponents } from "../dojo/contract-components";
import { type Config, EntityType, TickIds, TroopType } from "../types";
...
export class ClientConfigManager {
	...
	> = {};
	hyperstructureTotalCosts: Record<
		number,
		{ resource: ResourceTier; min_amount: number; max_amount: number }
	> = {};
	realmUpgradeCosts: Record<
		number,
	...
	private config!: Config;
	buildingOutputs: Record<number, number> = {};
	complexSystemResourceInputs: Record<
		number,
		{ resource: ResourcesIds; amount: number }[]
	> = {};
	complexSystemResourceOutput: Record<
		number,
		{ resource: ResourcesIds; amount: number }
	> = {};
	...
	simpleSystemResourceInputs: Record<
		number,
		{ resource: ResourcesIds; amount: number }[]
	> = {};
	simpleSystemResourceOutput: Record<
		number,
		{ resource: ResourcesIds; amount: number }
	> = {};
	laborOutputPerResource: Record<
		number,
		{ resource: ResourcesIds; amount: number }
	> = {};
	...
	realmUpgradeCosts: Record<
		...
		{ resource: ResourcesIds; amount: number }[]
	> = {};
	complexBuildingCosts: Record<
		number,
		{ resource: ResourcesIds; amount: number }[]
	> = {};
	simpleBuildingCosts: Record<
		number,
		{ resource: ResourcesIds; amount: number }[]
	> = {};
	structureCosts: Record<number, { resource: ResourcesIds; amount: number }[]> =
	...
	private initializeResourceWeights() {
		...
		for (const resourceType of Object.values(ResourcesIds).filter(
			Number.isInteger,
		)) {
			...
			this.resourceWeightsKg[Number(resourceType)] = gramToKg(
				Number(weightConfig?.weight_gram ?? 0),
			);
		}
	}
	...
	private initializeResourceProduction() {
		...
		for (const resourceType of Object.values(ResourcesIds).filter(
			...
			for (const resourceType of Object.values(ResourcesIds).filter(
				...
				const inputCount =
					productionConfig?.multiple_resource_burn_strategy
						.required_resources_count ?? 0;
				const entityId =
					productionConfig?.multiple_resource_burn_strategy
						.required_resources_id ?? 0;
				const inputs: { resource: ResourcesIds; amount: number }[] = [];
				const complexSystemResourceInputCount =
					productionConfig?.complex_input_list_count ?? 0;
				const complexSystemResourceInputEntityId =
					productionConfig?.complex_input_list_id ?? 0;
				const complexSystemResourceInputs: {
					resource: ResourcesIds;
					amount: number;
				}[] = [];
				...
				for (let index = 0; index < inputCount; index++) {
					...
					for (
						let index = 0;
						index < complexSystemResourceInputCount;
						index++
					) {
						const resource = getComponentValue(
							this.components.ResourceList,
							getEntityIdFromKeys([
								BigInt(complexSystemResourceInputEntityId),
								BigInt(index),
							]),
						);
						...
						if (productionInput) {
							...
							const amount = this.divideByPrecision(
								Number(productionInput.amount),
							);
					...
					if (resource) {
						...
						complexSystemResourceInputs.push({
				...
				if (laborBurnStrategy && laborBurnStrategy.resource_rarity > 0n) {
					this.resourceLaborOutput[Number(resourceType)] = {
						...
						depreciation_percent_num: Number(
							laborBurnStrategy.depreciation_percent_num,
						),
						depreciation_percent_denom: Number(
							laborBurnStrategy.depreciation_percent_denom,
						),
						wheat_burn_per_labor: this.divideByPrecision(
							Number(laborBurnStrategy.wheat_burn_per_labor),
						),
						fish_burn_per_labor: this.divideByPrecision(
							Number(laborBurnStrategy.fish_burn_per_labor),
						),
					};
				}
				const simpleSystemResourceInputCount =
					productionConfig?.simple_input_list_count ?? 0;
				const simpleSystemResourceInputEntityId =
					productionConfig?.simple_input_list_id ?? 0;
				...
				for (let index = 0; index < simpleSystemResourceInputCount; index++) {
					const resource = getComponentValue(
						this.components.ResourceList,
						getEntityIdFromKeys([
							BigInt(simpleSystemResourceInputEntityId),
							BigInt(index),
						]),
					);
					...
					if (resource) {
						const resource_type = resource.resource_type;
						const amount = this.divideByPrecision(Number(resource.amount));
						simpleSystemResourceInputs.push({
							resource: resource_type,
							amount,
						});
					}
				...
				const resourceOutput = Number(
					productionConfig?.realm_output_per_tick ?? 0n,
				);
		...
		this.complexSystemResourceInputs[Number(resourceType)] =
			complexSystemResourceInputs;
		this.complexSystemResourceOutput[Number(resourceType)] = {
			resource: Number(resourceType) as ResourcesIds,
			amount: this.divideByPrecision(
				Number(productionConfig?.output_per_complex_input) ?? 0,
			),
		};

		this.simpleSystemResourceInputs[Number(resourceType)] =
			simpleSystemResourceInputs;
		this.simpleSystemResourceOutput[Number(resourceType)] = {
			resource: Number(resourceType) as ResourcesIds,
			amount: this.divideByPrecision(
				Number(productionConfig?.output_per_simple_input) ?? 0,
			),
		};
		...
		this.resourceOutputRate[Number(resourceType)] = {
			resource: Number(resourceType) as ResourcesIds,
			realm_output_per_second: Number(
				productionConfig?.realm_output_per_second,
			),
			village_output_per_second: Number(
				productionConfig?.village_output_per_second,
			),
		};
	}
}
...
private
initializeHyperstructureTotalCosts();
{
	...
	for (const resourceTier of Object.values(ResourceTier).filter(
		Number.isInteger,
	)) {
		...
		const min_amount =
			Number(hyperstructureResourceConfig?.min_amount ?? 0) /
			RESOURCE_PRECISION;

		const max_amount =
			Number(hyperstructureResourceConfig?.max_amount ?? 0) /
			RESOURCE_PRECISION;

		hyperstructureTotalCosts.push({
			resource: resourceTier as ResourceTier,
			min_amount,
			max_amount,
		});
	}
...
private
initializeRealmUpgradeCosts();
{
	const worldConfig = getComponentValue(
		this.components.WorldConfig,
		getEntityIdFromKeys([WORLD_CONFIG_ID]),
	);
	const maxLevel =
		Number(worldConfig?.structure_max_level_config?.realm_max) || 0;

	for (let level = 1; level <= maxLevel; level++) {
		const levelConfig = getComponentValue(
			this.components.StructureLevelConfig,
			getEntityIdFromKeys([BigInt(level)]),
		);
		if (levelConfig) {
			const inputs: { resource: ResourcesIds; amount: number }[] = [];
			for (
				let index = 0;
				index < levelConfig.required_resource_count;
				index++
			) {
				const resource = getComponentValue(
					this.components.ResourceList,
					getEntityIdFromKeys([
						BigInt(levelConfig.required_resources_id),
						BigInt(index),
					]),
				);
				if (resource) {
...
{
	const buildingConfigsEntities = runQuery([
		Has(this.components.BuildingCategoryConfig),
	]);
	...
	for (const buildingConfigEntity of buildingConfigsEntities) {
		...
		if (buildingConfig) {
			...
			for (const buildingConfigEntity of buildingConfigsEntities) {
				...
				if (buildingConfig) {
					...
					const complexResourceCount =
						buildingConfig.complex_erection_cost_count || 0;
					...
					this.complexBuildingCosts[Number(buildingConfig.category)] =
						complexInputs;

					// Process simple building costs
					const simpleEntityId = buildingConfig.simple_erection_cost_id;
					const simpleResourceCount =
						buildingConfig.simple_erection_cost_count || 0;
					...
					this.simpleBuildingCosts[Number(buildingConfig.category)] =
						simpleInputs;
					...
					if (resourceType) {
						this.buildingOutputs[Number(buildingConfig.category)] =
							resourceType;
					}
				}
			}
		...
		this.structureCosts[StructureType.Hyperstructure] = [
			this.getHyperstructureConstructionCosts(),
		];
		...
		return this.getValueOrDefault(() => {
			...
			const baseStaminaCost =
				worldConfig?.troop_stamina_config?.stamina_travel_stamina_cost || 0;
			const biomeBonus =
				worldConfig?.troop_stamina_config?.stamina_bonus_value || 0;
			...
			switch (biome) {
				...
				case BiomeType.Grassland:
					return (
						baseStaminaCost +
						(troopType === TroopType.Paladin ? -biomeBonus : 0)
					);
		...
		getTroopConfig();
		{
			...
			const { troop_damage_config, troop_limit_config, troop_stamina_config } =
				worldConfig;
			...
			() => {
				...
				return {
					velords_fee_on_dpt_percent: Number(
						resourceBridgeFeeSplitConfig?.velords_fee_on_dpt_percent ?? 0,
					),
					velords_fee_on_wtdr_percent: Number(
						resourceBridgeFeeSplitConfig?.velords_fee_on_wtdr_percent ?? 0,
					),
					season_pool_fee_on_dpt_percent: Number(
						resourceBridgeFeeSplitConfig?.season_pool_fee_on_dpt_percent ?? 0,
					),
					season_pool_fee_on_wtdr_percent: Number(
						resourceBridgeFeeSplitConfig?.season_pool_fee_on_wtdr_percent ?? 0,
					),
					client_fee_on_dpt_percent: Number(
						resourceBridgeFeeSplitConfig?.client_fee_on_dpt_percent ?? 0,
					),
					client_fee_on_wtdr_percent: Number(
						resourceBridgeFeeSplitConfig?.client_fee_on_wtdr_percent ?? 0,
					),
					velords_fee_recipient:
						resourceBridgeFeeSplitConfig?.velords_fee_recipient ?? BigInt(0),
					season_pool_fee_recipient:
						resourceBridgeFeeSplitConfig?.season_pool_fee_recipient ??
						BigInt(0),
					realm_fee_dpt_percent: Number(
						resourceBridgeFeeSplitConfig?.realm_fee_dpt_percent ?? 0,
					),
					realm_fee_wtdr_percent: Number(
						resourceBridgeFeeSplitConfig?.realm_fee_wtdr_percent ?? 0,
					),
				};
			},
			{
		...
		return this.getValueOrDefault(() => {
			...
			switch (category) {
				...
				case CapacityConfig.Storehouse:
					capacityInGrams = Number(
						capacityConfig?.storehouse_boost_capacity ?? 0,
					);
					break;
		...
		getSpeedConfig(entityType: EntityType)
		: number
		return this.getValueOrDefault(() => {
		...
		return this.getValueOrDefault(
			() => {
				...
				return {
					timeBetweenSharesChange:
						hyperstructureConfig?.time_between_shares_change ?? 0,
					pointsPerCycle: Number(hyperstructureConfig?.points_per_cycle) ?? 0,
					pointsForWin: Number(hyperstructureConfig?.points_for_win) ?? 0,
					pointsOnCompletion:
						Number(hyperstructureConfig?.points_on_completion) ?? 0,
				};
			},
			{
		...
		getHyperstructureTotalContributableAmount(hyperstructureId: number)
		{
			const requiredAmounts =
				this.getHyperstructureRequiredAmounts(hyperstructureId);
			return requiredAmounts.reduce(
			(total, { amount, resource }) =>
				total + amount * this.getResourceRarity(resource),
			0,
		);
		}
		...
		getHyperstructureRequiredAmounts(hyperstructureId: number)
		{
			...
			for (const tier in ResourceTier) {
				...
				const resourcesInTier = GET_HYPERSTRUCTURE_RESOURCES_PER_TIER(
					resourceTierNumber,
					true,
				);
				const amountForTier = this.getHyperstructureRequiredAmountPerTier(
					resourceTierNumber,
					randomness,
				);
		...
		getHyperstructureRequiredAmountPerTier(
		...
		: number
		{
			...
			const additionalAmount = Number(
				randomness % BigInt(maxAmount - minAmount),
			);
		...
		return this.getValueOrDefault(() => {
			return (
				getComponentValue(
					this.components.WorldConfig,
					getEntityIdFromKeys([WORLD_CONFIG_ID]),
				)?.building_config?.base_population ?? 0
			);
		}, 0);
  ...
  getResourceOutputs(resourceType: number): number 
			...
			() => {
				...
				return {
					exploreWheatBurnAmount:
						Number(travelFoodCostConfig?.stamina_explore_wheat_cost) ?? 0,
					exploreFishBurnAmount:
						Number(travelFoodCostConfig?.stamina_explore_fish_cost) ?? 0,
					travelWheatBurnAmount:
						Number(travelFoodCostConfig?.stamina_travel_wheat_cost) ?? 0,
					travelFishBurnAmount:
						Number(travelFoodCostConfig?.stamina_travel_fish_cost) ?? 0,
				};
			},
			{
	...
	public
	getLaborConfig = (resourceId: number): LaborConfig | undefined => {
		const laborProducedPerResource =
			configManager.laborOutputPerResource[
				resourceId as keyof typeof configManager.laborOutputPerResource
			];
		const laborResourceOutput =
			configManager.resourceOutputRate[
				ResourcesIds.Labor as keyof typeof configManager.resourceOutputRate
			];
		const simpleSystemResourceInputs =
			configManager.simpleSystemResourceInputs[
				resourceId as keyof typeof configManager.simpleSystemResourceInputs
			];
		...
		return {
			laborProductionPerResource:
				laborProducedPerResource.amount / RESOURCE_PRECISION,
			laborBurnPerResourceOutput: laborBurnPerResourceOutput.amount,
			laborRatePerTick:
				laborResourceOutput.realm_output_per_second / RESOURCE_PRECISION,
