...
import type * as SystemProps from "../types/provider";
					...
					for (const batch of chunks) {
						...
						if (batch.length === 1) {
							...
							try {
								...
								const allCalls = batch.map(({ providerCall }) => {
								...
								const result = await this.provider.executeAndCheckTransaction(
									signer,
									flattenedCalls,
								);
...
export const buildVrfCalls = async ({
	...
	if (!vrfProviderAddress)
		throw new Error("VRF provider address is not defined");
...
export class EternumProvider extends EnhancedDojoProvider {
	...
	async executeAndCheckTransaction(
		...
		const transactionResult = await this.waitForTransactionWithCheck(
			tx.transaction_hash,
		);
		...
		if (isMultipleTransactions) {
			...
			txType =
				TransactionType[
					transactionDetails
						...
						.filter(
							(detail) => detail.contractAddress !== this.VRF_PROVIDER_ADDRESS,
						)[0]
			...
			txType =
				TransactionType[
					transactionDetails.entrypoint.toUpperCase() as keyof typeof TransactionType
				];
		}
	...
	async callAndReturnResult(
		signer: Account | AccountInterface,
		transactionDetails: DojoCall | Call,
	) {
		if (typeof window !== "undefined") {
			console.log({ signer, transactionDetails });
		}
	...
	public async bridge_start_withdraw_from_realm(
		props: SystemProps.BridgeStartWithdrawFromRealmProps,
	) {
		...
		const calls = resources.map((resource) => ({
			contractAddress: getContractByName(
				this.manifest,
				`${NAMESPACE}-resource_bridge_systems`,
			),
			entrypoint: "start_withdraw",
			calldata: [
				through_bank_id,
				from_realm_entity_id,
	...
	public async bridge_finish_withdraw_from_realm(
		props: SystemProps.BridgeFinishWithdrawFromRealmProps,
	) {
		const {
			donkey_resources,
			through_bank_id,
	...
	public async bridge_resources_into_realm(
		props: SystemProps.BridgeResourcesIntoRealmProps,
	) {
		const {
			resources,
			through_bank_id,
	...
	public async mint_resources(props: SystemProps.MintResourcesProps) {
		...
		return await this.executeAndCheckTransaction(props.signer, {
			contractAddress: getContractByName(
				this.manifest,
				`${NAMESPACE}-dev_resource_systems`,
			),
			entrypoint: "mint",
	...
	public async create_village(props: SystemProps.CreateVillageProps) {
		...
		if (
			this.VRF_PROVIDER_ADDRESS !== undefined &&
			Number(this.VRF_PROVIDER_ADDRESS) !== 0
		) {
			const requestRandomCall: Call = {
				contractAddress: this.VRF_PROVIDER_ADDRESS!,
				entrypoint: "request_random",
				calldata: [
					getContractByName(this.manifest, `${NAMESPACE}-village_systems`),
					0,
					signer.address,
				],
			};
	 ...
	 * ```
	 */
	public async create_multiple_realms(
		...
		const realmSystemsContractAddress = getContractByName(
			this.manifest,
			`${NAMESPACE}-realm_systems`,
		);
		...
		return await Promise.all(
			calls.map((call) => this.promiseQueue.enqueue(call)),
		);
	}
	 ...
	 * ```
	 */
	public async mint_and_settle_test_realm(
	 ...
	 * ```
	 */
	public async send_resources_multiple(
		...
		const call = this.createProviderCall(
			...
			calls.map((call) => ({
				...
				entrypoint: "send",
				calldata: [
					call.sender_entity_id,
					call.recipient_entity_id,
					call.resources.length / 2,
					...call.resources,
				],
			})),
		);
  ...
  public async create_building(props: SystemProps.CreateBuildingProps) {
	 ...
	 * ```
	 */
	public async change_bank_owner_fee(
		props: SystemProps.ChangeBankOwnerFeeProps,
	) {
		const { bank_entity_id, new_swap_fee_num, new_swap_fee_denom, signer } =
			props;
	 ...
	 * ```
	 */
	public async change_bank_bridge_fee(
		props: SystemProps.ChangeBankBridgeFeeProps,
	) {
		const {
			bank_entity_id,
			new_bridge_fee_dpt_percent,
			new_bridge_fee_wtdr_percent,
			signer,
		} = props;
	...
	public async guard_add(props: SystemProps.GuardAddProps) {
		...
		const call = this.createProviderCall(signer, {
			contractAddress: getContractByName(
				this.manifest,
				`${NAMESPACE}-troop_management_systems`,
			),
	 ...
	 * @returns Transaction receipt
	 */
	public async troop_structure_adjacent_transfer(
	 ...
	 * @returns Transaction receipt
	 */
	public async structure_troop_adjacent_transfer(
	 ...
	 * @returns Transaction receipt
	 */
	public async explorer_explorer_swap(
		props: SystemProps.ExplorerExplorerSwapProps,
	) {
		const {
			from_explorer_id,
			to_explorer_id,
			to_explorer_direction,
			count,
			signer,
		} = props;
		...
		if (
			...
			const requestRandomCall: Call = {
				...
				calldata: [
					getContractByName(
						this.manifest,
						`${NAMESPACE}-troop_movement_systems`,
					),
					0,
					signer.address,
				],
	 ...
	 * @returns Transaction receipt
	 */
	public async attack_explorer_vs_explorer(
		...
		const call = this.createProviderCall(signer, {
			contractAddress: getContractByName(
				this.manifest,
				`${NAMESPACE}-troop_battle_systems`,
			),
	 ...
	 * @returns Transaction receipt
	 */
	public async attack_explorer_vs_guard(
	 ...
	 * @returns Transaction receipt
	 */
	public async attack_guard_vs_explorer(
	...
	public async mint_starting_resources(
		props: SystemProps.MintStartingResources,
	) {
	...
	public async transfer_guild_ownership(
		props: SystemProps.TransferGuildOwnership,
	) {
	...
	public async remove_player_from_whitelist(
		props: SystemProps.RemovePlayerFromWhitelist,
	) {
	...
	public async set_starting_resources_config(
		props: SystemProps.SetStartingResourcesConfigProps,
	) {
  ...
  public async set_starting_resources_config(props: SystemProps.SetStartingResourcesConfigProps) {
    const { realmStartingResources, villageStartingResources, signer } = props;
		...
		return await this.executeAndCheckTransaction(signer, {
			contractAddress: getContractByName(
				this.manifest,
				`${NAMESPACE}-config_systems`,
			),
			entrypoint: "set_starting_resources_config",
			calldata: [
				startingResources.length,
				...startingResources.flatMap(({ resource, amount }) => [
					resource,
					amount,
				]),
			],
	...
	public async set_travel_food_cost_config(
		props: SystemProps.SetTravelFoodCostConfigProps,
	) {
		const {
			config_id,
			unit_type,
	...
	public async set_resource_bridge_fees_config(
		props: SystemProps.SetResourceBridgeFeesConfigProps,
	) {
		const {
	...
	public async set_agent_controller(
		props: SystemProps.SetAgentControllerProps,
	) {
	...
	public async set_donkey_speed_config(
		props: SystemProps.SetDonkeySpeedConfigProps,
	) {
	...
	public async set_resource_weight_config(
		props: SystemProps.SetWeightConfigProps,
	) {
		const { calls, signer } = props;
	...
	public async set_production_config(
		props: SystemProps.SetProductionConfigProps,
	) {
		...
		const productionCalldataArray = calls.map((call) => {
			return {
				...
				calldata: [
					...
					...call.predefined_resource_burn_cost.flatMap(
  ...
  public async set_resource_factory_config(props: SystemProps.SetResourceFactoryConfigProps) {
	...
	public async set_bank_config(props: SystemProps.SetBankConfigProps) {
		const { lp_fee_num, lp_fee_denom, owner_fee_num, owner_fee_denom, signer } =
			props;
	...
	public async set_resource_bridge_whitlelist_config(
		props: SystemProps.SetResourceBridgeWhitelistConfigProps,
	) {
		const { resource_whitelist_configs, signer } = props;

		const calldata = resource_whitelist_configs.map(
			({ token, resource_type }) => ({
				contractAddress: getContractByName(
					this.manifest,
					`${NAMESPACE}-config_systems`,
	...
	public async set_battle_config(props: SystemProps.SetBattleConfigProps) {
		const { signer, regular_immunity_ticks, hyperstructure_immunity_ticks } =
			props;
	...
	public async set_structure_level_config(
		props: SystemProps.setRealmUpgradeConfigProps,
	) {
		const { calls, signer } = props;
			...
			calls.map((call) => {
				return {
					...
					calldata: [
						...
						...call.cost_of_level.flatMap(({ resource, amount }) => [
							resource,
							amount,
						]),
					],
	...
	public async set_mercenaries_name_config(
		props: SystemProps.SetMercenariesNameConfigProps,
	) {
		const { name, signer } = props;
	...
	public async set_structure_max_level_config(
		props: SystemProps.SetStructureMaxLevelConfigProps,
	) {
	...
	public async set_building_category_config(
		props: SystemProps.SetBuildingCategoryConfigProps,
	) {
		const {
			building_category,
			cost_of_building,
		...
		return await this.executeAndCheckTransaction(signer, {
			...
			calldata: [
				...
				...cost_of_building.flatMap(({ resource, amount }) => [
					resource,
					amount,
				]),
				population_cost,
  ...
  public async set_building_category_config(props: SystemProps.SetBuildingCategoryConfigProps) {
	...
	public async set_hyperstructure_config(
		props: SystemProps.SetHyperstructureConfig,
	) {
		const {
	...
	public async initialize(props: SystemProps.InitializeHyperstructureProps) {
		...
		const call = this.createProviderCall(signer, {
			contractAddress: getContractByName(
				this.manifest,
				`${NAMESPACE}-hyperstructure_systems`,
			),
	...
	public async contribute_to_construction(
	...
	public async end_game(props: SystemProps.EndGameProps) {
		const {
			signer,
			hyperstructure_contributed_to,
			hyperstructure_shareholder_epochs,
		} = props;
	...
	public async register_to_leaderboard(
		props: SystemProps.RegisterToLeaderboardProps,
	) {
		const {
			signer,
			hyperstructure_contributed_to,
	...
	public async claim_leaderboard_rewards(
	...
	public async set_stamina_refill_config(
		props: SystemProps.SetStaminaRefillConfigProps,
	) {
	...
	public async set_settlement_config(
		props: SystemProps.SetSettlementConfigProps,
	) {
	...
	public async attach_lords(props: SystemProps.AttachLordsProps) {
		const { amount, signer, token_id, season_pass_address, lords_address } =
			props;
	 ...
	 * ```
	 */
	public async burn_other_resources_for_labor_production(
  ...
  public async burn_resource_for_labor_production(props: SystemProps.BurnOtherResourcesForLaborProductionProps) {
	 ...
	 * ```
	 */
	public async burn_labor_resources_for_other_production(
		props: SystemProps.BurnLaborResourcesForOtherProductionProps,
	) {
		const { from_entity_id, labor_amounts, produced_resource_types, signer } =
			props;
  /**
   * Burn labor resources to produce other resources
   *
